pipeline {
  ...
  environment{
      ...
      DOCKER_ARTIFACTORY = "shruti/weather-app"
  ...
  stages {
    ...
    stage('Push to DockerHub') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'docherHub', passwordVariable: 'docherHubUserToken', usernameVariable: admin')]) {
          sh "echo ${env.docherHubUserToken}  | docker login -u ${env.docherHubUserId} --password-admin"
